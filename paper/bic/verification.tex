...
% TODO (James): Over-complicating formulation of what should be a simple sentence? :)
...
they are also the most difficult. It is worth noting that the RMSE is largest
in all cases for our method, which is due to our model being based on
continuous probabilities rather than a binary demarcation. This means that the
RMSE will increase even when our values are only slightly wrong, in an
otherwise correctly predicted region.
