...
brilliance and collimation which gives a very clear signal. Its mono-energetic characteristics also
means that X-ray artifacts are reduced, in particular beam-hardening is minimized.
...
reconstructed using the Python framework PyHST \citep{pyhst}.
...
specifically $2^5=32$ was used in our case. A full image sample is further split into 4-6 sub-volumes
through the height of the implant. This gives a size of $(3456,3456,846)$ pixels per sub-volume,
where the last axis gets stacked for a full volume. Before being stacked, we need to volume match
any overlapping regions. The overlaps are only shifted in the last dimension. This made it possible
to find the best overlap by simply minimizing the pixel-wise eucledian distance between the volumes.
The size of the shifted volumes goes from $846 \rightarrow 810$.

\aleksandar{Edge artifacts from volume matching is probably a good idea to include. It can
be seen on the images, and contributes to the misrepresentation of voxels. Also motivate why
it was cut into subvolumes to begin with. Also how this was done before acquisition.
How were the samples secured in order not to be physically shifted too much etc.}
...
components \citep{srnoise}. This contributes to induce two distinct effects typically seen as a
result of beam-hardening. Those are in particular streaks and and cupping artifacts from high
density regions.

%\carl{Should be emphasized more. \textbackslash ref\{initial-beam-claim\} claims that there are no
% beam-hardening effects, and I think this reads as "well, yes, but no"}
