...
further split into 4-6 sub-volumes through the height of the implant. This gives a size of
...
In image \cref{fig:3viewsample} (YZ and XZ planes) we can actually see the edge artifacts from the
volume matched subvolumes.
...
imperfections in the scanner setup. It can be from uncalibrated or defect adjacent detector elements.
For synchrotron radiation sources it can also occur from shifts and vibrations in the monochromator
...
are dependent on the voxel size and are mentioned briefly by Neldam et al.}

For SR$\mu$CT a high photon flux allows for very short exposure times \citep{srexptime}.
This can help counter Poisson noise from suboptimal photon count \citep{srnoise}.
