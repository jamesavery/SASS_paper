...
\textit{field} representations: Eucledian Distance Transform (EDT) and Diffusion.

%EDT is good overall, but difficult close to the implant.
EDT computes each voxel as the Eucledian distance to the implant.  This makes for a generally good
...
\subsection{Evaluating bone-implant contact and blood-implant contact}
...
\begin{figure}
  ...
  \end{tabular}
  \caption{XY slices of the original tomography (top), and the segmentation (bottom) with yellow depicting bone and red depicting soft-tissue. Note how even though it is hard to see, the segmentation seems to accurately segment the materials close to the implant.}
...
\begin{figure}
  ...
  \end{tabular}
  \caption{ZY slices of the segmentation seen far away (top) and zoomed in (bottom). Yellow depicts bone and red depicts soft-tissue. Note that the segmentation segments the materials close to the implant, even in the grooves of the implant..}
