\section{Method}
\label{sec:method}
In this section, we will present the method for exploiting spatial correlation
based on the 2D histograms of the tomographies. We will refer to this approach
as \textit{Spatial Analysis for SRµCT Segmentation} or SASS.

Each scanned sample consists of multiple subvolumes, which are combined into a
single coherent 3D image of the full sample. This is done by shifting along the
$Z$-axis until the square 3D image differences are minimized over the overlap
volume, producing the best volume match. The particular sample we will be
working with has resolution $3360 \times 3456 \times 3456$ and with three
slices shown in \Cref{fig:3viewsample}.
...
For each row in the histogram, we apply adaptive Otsu's thresholding to find the
...
\begin{figure}
    ...
    \end{tabular}
    \caption{
	Visualization of (a) the adaptive Otsu thresholding (orange) and the
	fitted curve (blue) applied to the combined field histogram, and (b)
	the probability distributions for the two materials, red being soft
	tissue and yellow being bone.
    }
    \label{fig:otsu}
\end{figure}
...
\begin{figure}
    ...
    \begin{adjustbox}{width=0.6\linewidth}
        \begin{tikzpicture}
            ...
            \node[draw, fill=black!10, rounded corners, rectangle] (otsu) at (0,-7) {Adaptive Otsu thresholding};
