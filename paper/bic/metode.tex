...
If we look at a 1D-histogram of the voxel intensity of a tomography, as shown in~\Cref{fig:1d-hist}, then even though we are able to distinguish different distributions, we see that there is a large overlap leading to global thresholding being infeasible, at least for the distributions in the lower half of the histogram.
...
\begin{figure}
    ...
    \caption{1-dimensional histogram of the voxel intensities of a tomography. The red line is the soft tissue peak, the green line is the bone peak, and the orange line is the coarse implant threshold.}
    \label{fig:1d-hist}
\end{figure}
...
From our work in another paper, we are able to obtain two masks: an implant mask and a bone mask.

Since the intensety of the implant material is radically different from the rest of the sample, obtaining the rough implant mask through global thresholding is trivial, as the orange line in~\Cref{fig:1d-hist} shows.
This mask is then refined along with the internal gaps closed, giving us the implant mask. 
Using this mask, we are also able to remove the air making up half of the sample cylinder (depicted in the XY and YZ cross sections of~\Cref{fig:3viewsample}), as the implant mask can show us the orientation of the implant. This removes the leftmost peak from the 


In order to remove the gaps within the implant, we apply a closing morphology filter. The implant mask is then used to compute the principal axes from the implant in order to compute what is considered air and what is considered resin. From this information, we obtain the bone mask. These two masks are applied throughout the steps to filter out noise in the tomography.
