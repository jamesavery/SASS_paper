...
\begin{algorithm}
    ...
    \begin{algorithmic}
        \Function {Diffusion} {$voxels[n_z*n_y*n_x], repitions, kernel[2*k+1]$}
            ...
            \State \textbf{allocate} $tmp_0[n_z*n_y*n_z], tmp_1[n_z*n_y*n_x]$
            \State $tmp_0[:] \gets voxels[:]$
                ...
                \For {$dim$ \textbf{in} $dimensions$}
                    \For {$z,y,x$ \textbf{in} $0{:}n_z,0{:}n_y,0{:}n_x$}
                        ...
                        \For {$i$ \textbf{in} $i_{start}:i_{end}$}
                            ...
                            \State $tmp_1[index] \gets tmp_0[i_{offset}] * kernel[i+k]$
                        \EndFor
                    \EndFor
                    \State $tmp_0[:] \gets tmp_1[:]$
                \EndFor
            \EndFor
            \Return $tmp_0$
        \EndFunction
