...
titanium implant and biological tissue. We can group the voxels according to their distance to the implant using the
...
the same distance from the implant receives much fewer contributions. To resolve brightening of voxels very close to the implant, it thus makes sense
...
%          \draw[-latex,blue,densely dotted, very thick] (N2 \p.south) -- (N2 \p.north);
        ...
        \draw[red, line width=0.5mm] (0.65,1.20) circle (0.05cm);
...
\begin{enumerate}
    ...
    \item This blood network is used alongside the segmented bone to produce the final metric.
\end{enumerate}
